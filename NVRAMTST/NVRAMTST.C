...
* 28/4/94. DJLB added facility to read or write NVRAM contents from or to a
*			disk file.
****************************************************************************/
...
extern unsigned char	*sat_details_ptr[ NUM_SATELLITES ],
...
extern unsigned int	*current_sat_num_ptr,
			*orbit_start_ptr,
			*current_star_num_ptr;

extern void 		*sw_limits_ptr; /* Software limits */
extern void		*stow_pos_ptr;	/* Stow positions */

/* Controller algorithm constants */
extern void 		*con_ptr [ 2 ];	/* two axes of controller constants*/

#define BOOLEAN char
#define NO 0
#define YES 1
extern unsigned int   size_of_nvstore;
...
char  *chPtr;
	...
	gotoxy( 5 , 1 );
	cprintf("ATCU NVRAM Test Program. $Revision:   1.1.1.0  $ ");
	...
	cprintf("9. ");
	lowvideo();
	cprintf("Copy NVRAM to or from disk.");
...
intelsat_parameters intelsat;
	...
	switch ( command )
	{
		...
		case 'p':
			printf("Enter the satellite number: ");
			scanf("%i" , &temp_int );
...
char	c;
	...
	switch ( command )
	{
		...
		case 'n' :
		    ...
		    printf("\nint_positive_limit: %lf ",controller_parameter.int_positive_limit);
		    printf("\nint_negative_limit: %lf ",controller_parameter.int_negative_limit);
		    printf("\nsign:        %d ",controller_parameter.sign);
		    printf("\nmit_alpha:   %lf ",controller_parameter.mit_alpha);
		    printf("\nmit_beta:    %lf ",controller_parameter.mit_beta);
		    printf("\nmit_maxref:  %lf ",controller_parameter.mit_maxref);
		    printf("\nmit_maxincr: %lf ",controller_parameter.mit_maxincr);
...
int	i;
	...
	cprintf("calibration_coeff_ptr = %0lX" , (unsigned long)nv_calibration_coefficients );
...
int	i,j;
	...
	`s' = no NVRAM exists.
    */
    	if(argc > 1 && *argv[1] == 's') nvram_sim = 1;
    	else nvram_sim = 0;	 
...
case 9 :
	...
	} while ( main_command != 10 );

	clrscr();
    printf("Program terminated.\n");
}

/* get_nvram_sim() is a sequencer-provided utility, in ATCU code. It is
	duplicated here to allow nvramtst() to provide the same utility
	to the nvram code. */
...
void copy_nvram( void )
{
char	chInput, chPathname[34];
static BOOLEAN boRepeat, boAllOK;
int   inHandle;
unsigned int uinBytes;
  ...
  while( boRepeat == YES )
  {
	...
	clreol();
	lowvideo();
	cputs("Write a file or read a file into NVRAM (W/R) :");
	highvideo();
	chInput = tolower( getche() );
	boRepeat = NO;

	if( chInput == 'w' || chInput == 'r' )
	{
	  /* Get a name for the file to be written or read. */
	  gotoxy(10,6);
	  lowvideo();
	  cputs("Pathname for file: ");
	  highvideo();
	  chPathname[0] = 32;
	  cgets( chPathname );
	  gotoxy(20,8);
	  ...
	  if( chInput == 'w' )
	  {
		inHandle = open( &chPathname[2], O_WRONLY | O_CREAT | O_BINARY );
		if( inHandle < 0 )
		{
			gotoxy(20,9);
			lowvideo();
		...
		else
		{
			...
			{
				cputs( "File write failure. Try again, or quit? (T/Q):");
				chInput = tolower( getche() );
				if( chInput == 't' )
					boRepeat = YES;
				else
	  ...
	  {
		inHandle = open( &chPathname[2], O_RDONLY | O_BINARY );
		if( inHandle < 0 )
		{
			gotoxy(20,9);
			lowvideo();
			cputs("Cannot open such a file. Try again, or quit? (T/Q): ");
			chInput = tolower( getche() );
			if( chInput == 't' )
				boRepeat = YES;
			else
		...
		else
		{
			gotoxy(20,9);
			cputs("Reading file...");
			/* Write to 6M21 control port to allow access. This presently
			involves allowing write access to both banks and disabling the
			global read/write shutdown bit. */
			outportb( 0x200 , 0xe0 );

			uinBytes = read( inHandle, (void *)NVRAM_BASE_ADDR,
													size_of_nvstore );
			gotoxy(15,11);
			lowvideo();
			...
			if( uinBytes == size_of_nvstore )
			{
				cprintf("%d bytes written to NVRAM from file %s",
												uinBytes, chPathname);
				boRepeat = NO;
			}
			else
			{
				cputs( "File read failure.Try again, or quit? (T/Q): ");
				chInput = tolower( getche() );
				if( chInput == 't' )
					boRepeat = YES;
				else
  ...
  {
	gotoxy(20,13);
	cputs("Hit any key to return to main menu.");
	getch();
  }
}