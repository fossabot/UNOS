...
#include "scrext.h"
#include "seqscr.h"
#include "nvramext.h" // Import select_new_beacon(), clear_nvram_error()
	...
	switch ( key_pressed ) {
		...
		case CONTROL_A:			/* Adjust azimuth to left ^A */
			switch ( cp_data.mode ) {
				...
				case POSITION_MODE:
					az_pos = fmod ( cp_data.az_cmd - increment, 360.0 );
					if ( az_pos < 0 )
				...
				case POSITION_RADEC_MODE:
					az_pos = fmod ( cp_data.ra_cmd - increment, 24.0 );
					if ( az_pos < 0 )
